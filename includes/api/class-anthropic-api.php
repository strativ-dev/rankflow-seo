 ...
 * @author     Strativ AB
 */
class MPSEO_Anthropic_API extends MPSEO_API_Base
{
	...
	public function generate_meta_tags($title, $content, $focus_keyword = '')
	{
		...
		if (empty($this->api_key)) {
			return new WP_Error('no_api_key', __('Anthropic API key is not configured', 'metapilot-smart-seo'));
		}
			...
			if (isset($error_data['error'])) {
				...
				if (isset($error['type']) && isset($error['message'])) {
					$error_message = sprintf(
						...
						__('Anthropic API Error (%1$s): %2$s', 'metapilot-smart-seo'),
						$error['type'],
						$error['message']
					);
				} else {
					$error_message = isset($error['message']) ? $error['message'] : __('Unknown API error', 'metapilot-smart-seo');
				}
			} else {
				$error_message = sprintf(
					/* translators: %d: HTTP status code */
					__('Anthropic API request failed with status code: %d', 'metapilot-smart-seo'),
					$status_code
				);
			}
		...
		if (JSON_ERROR_NONE !== json_last_error()) {
			return new WP_Error('json_error', __('Failed to parse API response', 'metapilot-smart-seo'));
		}

		// Extract content from response - Anthropic format.
		if (!isset($data['content']) || !is_array($data['content']) || empty($data['content'])) {
			return new WP_Error('invalid_response', __('Invalid response structure from Anthropic', 'metapilot-smart-seo'));
		}
		...
		if (empty($content_text)) {
			return new WP_Error('no_content', __('No text content in API response', 'metapilot-smart-seo'));
		}
