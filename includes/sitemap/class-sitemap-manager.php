...
class MPSEO_Sitemap_Manager
{
	...
	private function render_xsl_stylesheet()
	{
		...
		$xsl_file = MPSEO_PLUGIN_DIR . 'assets/xml/sitemap.xsl';

		if ( $wp_filesystem->exists( $xsl_file ) ) {
			$content = $wp_filesystem->get_contents( $xsl_file );
			// Replace placeholder with sanitized site name.
			$content = str_replace( '{{SITE_NAME}}', esc_html( get_bloginfo( 'name' ) ), $content );
			echo $content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSL template file content.
		}
