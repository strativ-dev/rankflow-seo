...
<div class="redirect-form-container" style="max-width: 800px;">
    ...
    <form method="post" action="" class="redirect-form">
            ...
            <tr>
                ...
                <td>
                    <input type="text" id="source_url" name="source_url"
                        value="<?php echo esc_attr($rankflow_seo_redirect->source_url); ?>" class="large-text" required
            ...
            <tr>
                ...
                <td>
                    <input type="text" id="target_url" name="target_url"
                        value="<?php echo esc_attr($rankflow_seo_redirect->target_url); ?>" class="large-text" required
            ...
            <tr>
                ...
                <td>
                    <select id="redirect_type" name="redirect_type" class="regular-text">
                        <option value="301" <?php selected($rankflow_seo_redirect->redirect_type, '301'); ?>>
                            301 - <?php esc_html_e('Permanent', 'rankflow-seo'); ?>
                        </option>
                        <option value="302" <?php selected($rankflow_seo_redirect->redirect_type, '302'); ?>>
                            302 - <?php esc_html_e('Temporary', 'rankflow-seo'); ?>
                        </option>
                        <option value="307" <?php selected($rankflow_seo_redirect->redirect_type, '307'); ?>>
                            307 - <?php esc_html_e('Temporary (Preserve Method)', 'rankflow-seo'); ?>
                        </option>
                        <option value="410" <?php selected($rankflow_seo_redirect->redirect_type, '410'); ?>>
                            410 - <?php esc_html_e('Gone', 'rankflow-seo'); ?>
                        </option>
                        <option value="451" <?php selected($rankflow_seo_redirect->redirect_type, '451'); ?>>
                            451 - <?php esc_html_e('Unavailable For Legal Reasons', 'rankflow-seo'); ?>
                        </option>
                    </select>
                    <p class="description">
        ...
        <div class="redirect-examples"
            ...
            <h4 style="margin-top: 0;"><?php esc_html_e('Examples:', 'rankflow-seo'); ?></h4>
            <ul style="margin: 0;">
                ...
                <li><strong><?php esc_html_e('External:', 'rankflow-seo'); ?></strong> /blog → https://blog.example.com
                </li>
                <li><strong><?php esc_html_e('Regex:', 'rankflow-seo'); ?></strong> /category/(.*) → /new-category/$1
                </li>
            </ul>
        </div>
    ...
    jQuery(document).ready(function ($) {
        ...
        $('.redirect-form').on('submit', function (e) {
            ...
            if (!sourceUrl || sourceUrl.trim() === '') {
                alert('<?php echo esc_js(__('Please enter a source URL.', 'rankflow-seo')); ?>');
                e.preventDefault();
                return false;
            }
            ...
            if (redirectType !== '410' && redirectType !== '451') {
                if (!targetUrl || targetUrl.trim() === '') {
                    alert('<?php echo esc_js(__('Please enter a target URL.', 'rankflow-seo')); ?>');
                    e.preventDefault();
                    return false;
                }
            }
